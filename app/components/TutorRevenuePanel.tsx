...
function StatTile({
  ...
  return (
    <div className="bg-card rounded-2xl p-4 border border-border shadow-[0_2px_8px_rgba(0,0,0,0.06)]">
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <StatTile
          icon={CreditCard}
          label="2026년 누적 수익"
          value={fmt(totalThis) + "원"}
          sub={fmtWon(totalThis)}
          trend={trend}
        />
        ...
        <StatTile
          icon={TrendingUp}
          label="레슨당 평균 수익"
          value={fmt(avgPerLesson) + "원"}
          sub={fmtWon(avgPerLesson)}
        />
        <StatTile
          icon={Users}
          label="이번 달 수익"
          value={fmt(thisMonthRev) + "원"}
          sub="2026년 8월"
          trend={trend}
        />
      </div>
          ...
          <div className="w-full h-56">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={YEARLY} margin={{ top: 4, right: 8, left: 0, bottom: 0 }}>
                ...
                />
                <Tooltip
                  content={<ChartTooltip unit="원" />}
                  cursor={{ stroke: C_PRIMARY, strokeWidth: 1, strokeDasharray: "4 2" }}
                />
                <Area
                  dataKey="revenue"
                  name="연간 수익"
                  ...
                  strokeWidth={2}
                  fill="url(#revGrad)"
                  dot={{ r: 5, fill: C_PRIMARY, strokeWidth: 2, stroke: "#fff" }}
                  activeDot={{ r: 7 }}
                />
              </AreaChart>
            </ResponsiveContainer>
            ...
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={MONTHLY_2026} margin={{ top: 4, right: 8, left: 0, bottom: 0 }}>
                ...
                <Line
                  ...
                  stroke={C_ACCENT}
                  strokeWidth={2}
                  dot={{ r: 4, fill: C_ACCENT, strokeWidth: 2, stroke: "#fff" }}
                  activeDot={{ r: 6 }}
                />
              </LineChart>
            </ResponsiveContainer>
        ...
        {[...MONTHLY_2026].reverse().map((d, i) => (
          <div
            ...
            className={`grid grid-cols-[1fr_auto_auto_auto] items-center gap-x-4 px-5 py-3 ${
              i !== MONTHLY_2026.length - 1 ? "border-b border-border" : ""
            }`}
          >
            ...
            <span className="text-sm text-foreground text-right">
              {Math.round(d.revenue / d.lessons).toLocaleString()}원
            </span>
          </div>
        ))}
      </div>
