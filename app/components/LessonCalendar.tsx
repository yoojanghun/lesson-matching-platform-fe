...
export default function LessonCalendar({ payments, bookings }: Props) {
  ...
  return (
    <div className="space-y-4">
      {/* 월 네비게이션 */}
      <div className="bg-card border border-border rounded-2xl shadow-[0_2px_8px_rgba(0,0,0,0.06)] overflow-hidden">
          ...
          {selectedLessons.length === 0 ? (
            ...
            <div>
              {selectedLessons.map((l, i) => {
                ...
                const isPending =
                  (l.source === "payment" && l.payStatus === "unpaid") ||
                  (l.source === "booking" && l.bookStatus === "pending");
                return (
                  <div
                    key={i}
                    className={`flex items-center gap-4 px-5 py-4 ${
                      i !== selectedLessons.length - 1 ? "border-b border-border" : ""
                    }`}
                  >
