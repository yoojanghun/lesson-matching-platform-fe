...
const AUTO_REPLIES = [
  ...
  "물론이죠! 언제든지 문의 주세요 😊",
  "레슨 관련해서 더 궁금한 점 있으시면 편하게 말씀해 주세요.",
    ...
    <div className="flex flex-col h-full bg-card">
      ...
      <div className="flex items-center gap-3 px-4 py-3.5 border-b border-border bg-card shrink-0">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-sm font-bold text-foreground leading-none">{tutorName} 튜터</p>
          <p className="text-[11px] text-muted-foreground mt-0.5">{tutorSubject}</p>
        </div>
        <button
          onClick={onClose}
          className="p-1.5 rounded-lg hover:bg-muted transition-colors cursor-pointer text-muted-foreground shrink-0"
        >
          <X size={16} />
        </button>
      </div>
  ...
  return (
    <>
      ...
      <button
        ...
        className={`fixed bottom-6 right-6 z-50 w-14 h-14 rounded-full shadow-2xl flex items-center justify-center transition-all duration-300 cursor-pointer border-2 border-white/20 ${
          open
