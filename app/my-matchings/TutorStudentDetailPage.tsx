...
export default function TutorStudentDetailPage({ matching, onBack, onUpdateFee }: Props) {
  ...
  const handleSaveFee = () => {
    ...
    if (fee > 0) {
      onUpdateFee(matching.id, fee);
      setEditingFee(false);
      showToast(`1회 레슨비가 ${fee.toLocaleString()}원으로 수정되었습니다.`);
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        <button
          onClick={onBack}
          className="w-9 h-9 flex items-center justify-center rounded-xl hover:bg-muted transition-colors cursor-pointer shrink-0 border border-border"
        >
          <ArrowLeft size={18} />
        </button>
        <div>
      ...
      {/* 학생 정보 + 레슨비 카드 */}
      <div className="bg-card rounded-2xl border border-border shadow-[0_2px_8px_rgba(0,0,0,0.06)] p-5 space-y-5">
        {/* 기본 정보 */}
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <div
              className={`rounded-xl p-3 text-center border ${
                monthUnpaid > 0 ? "bg-amber-50 border-amber-100" : "bg-muted/40 border-border/50"
              }`}
            >
      ...
      {/* 전체 레슨 내역 */}
      <div className="bg-card rounded-2xl border border-border overflow-hidden shadow-[0_2px_8px_rgba(0,0,0,0.06)]">
        ...
        {lessons.length === 0 ? (
          ...
          [...lessons].reverse().map((l, i) => (
            <div
              key={l.id}
              className={`grid grid-cols-[1fr_auto_auto_110px] items-center gap-x-4 px-5 py-3.5 ${
              ...
              return (
                <div>
                  ...
                  <div className="px-5 py-4 border-t border-border">
                    <button
                      onClick={switchToAdd}
                      className="w-full flex items-center justify-center gap-2 py-2.5 bg-primary text-primary-foreground rounded-xl text-sm font-bold hover:bg-primary/90 transition-colors cursor-pointer shadow-sm"
                    >
                      <Plus size={15} /> 레슨 추가
                    </button>
                  </div>
              ...
              <div className="p-5 space-y-4">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <button
                    ...
                    className={`relative w-12 h-6 rounded-full transition-colors cursor-pointer ${
                      modalPaid ? "bg-emerald-500" : "bg-muted-foreground/30"
                    }`}
                  >
                    <span
                      className={`absolute top-1 w-4 h-4 rounded-full bg-white shadow transition-transform ${
                        modalPaid ? "translate-x-7" : "translate-x-1"
                      }`}
                    />
                  </button>
                </div>
                ...
                <div className="flex gap-2 pt-1">
                  ...
                  <button
                    ...
                    className="flex-1 py-2.5 bg-primary text-primary-foreground rounded-xl text-sm font-bold hover:bg-primary/90 transition-colors cursor-pointer shadow-sm"
                  >
                    확정
                  </button>
                </div>
      ...
      {(() => {
        ...
        for (const l of lessons) {
          ...
          if (!map.has(ym)) {
            map.set(ym, { collected: 0, unpaid: 0, count: 0 });
            mapOrder.push(ym);
          }
        ...
        return (
          <>
            {/* 월별 수익 바 차트 */}
            <div className="bg-card rounded-2xl border border-border shadow-[0_2px_8px_rgba(0,0,0,0.06)] p-5">
            ...
            <div className="bg-card rounded-2xl border border-border shadow-[0_2px_8px_rgba(0,0,0,0.06)] p-5">
              ...
              <ResponsiveContainer width="100%" height={160}>
                <LineChart data={chartData} margin={{ top: 4, right: 16, left: 0, bottom: 0 }}>
                  ...
                  />
                  <Tooltip
                    content={<ChartTooltip unit="회" />}
                    cursor={{ stroke: C_ACCENT, strokeWidth: 1, strokeDasharray: "4 2" }}
                  />
                  <Line
                    dataKey="count"
                    name="레슨 수"
