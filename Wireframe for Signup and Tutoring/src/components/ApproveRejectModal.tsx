...
export default function ApproveRejectModal({ matching, onClose, onApprove, onReject }: Props) {
  ...
  const handleApprove = () => {
    setAction("approve");
    setTimeout(() => { onApprove(matching.id); onClose(); }, 900);
  };

  const handleReject = () => {
    setAction("reject");
    setTimeout(() => { onReject(matching.id); onClose(); }, 900);
  };
      ...
      <div className="relative bg-card rounded-3xl shadow-2xl w-full max-w-md overflow-hidden">
        ...
        <div className="px-6 pb-6 flex gap-3">
          ...
          <button
            ...
            disabled={action !== null}
            className={`flex-1 py-3 rounded-2xl text-sm font-bold transition-all cursor-pointer flex items-center justify-center gap-2 ${
              action === "approve"
