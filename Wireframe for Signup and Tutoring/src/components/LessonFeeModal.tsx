...
interface TutorMatchingWithFee {
  id: number;
  student: string;
  subject: string;
  date: string;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-card rounded-3xl shadow-2xl w-full max-w-md overflow-hidden">
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-border">
          ...
          <button onClick={onClose} className="w-8 h-8 flex items-center justify-center rounded-xl hover:bg-muted transition-colors cursor-pointer">
          ...
          <div className="space-y-2">
            ...
            {fee > 0 && (
              <p className="text-xs text-muted-foreground px-1">
                기본 금액 <span className="font-semibold text-foreground">{fmt(defaultFee)}원</span>에서 자유롭게 수정하세요
              </p>
            )}
          </div>
        </div>
        ...
        <div className="px-6 pb-6">
          <button
            ...
            className={`w-full py-3 rounded-2xl text-sm font-bold transition-all cursor-pointer flex items-center justify-center gap-2 ${
              saved
          ...
          >
            {saved ? <><CheckCircle2 size={15} /> 저장 완료!</> : "레슨비 확정"}
          </button>
        </div>
      </div>
    </div>
