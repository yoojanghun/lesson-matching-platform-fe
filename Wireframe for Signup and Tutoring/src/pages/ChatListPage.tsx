...
export default function ChatListPage() {
  ...
  const filtered = convs.filter(
    (c) => c.tutorName.includes(search) || c.tutorSubject.includes(search)
  );
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">튜터와의 대화를 확인하세요</p>
        </div>
        {totalUnread > 0 && (
      ...
      {/* 채팅 패널: 좌 목록 + 우 채팅창 */}
      <div className="bg-card rounded-2xl shadow-[0_2px_8px_rgba(0,0,0,0.06)] overflow-hidden border border-border"
        style={{ height: "600px" }}>
        <div className="flex h-full">

          {/* ── 왼쪽: 대화 목록 ── */}
          <div className={`flex flex-col border-r border-border shrink-0 ${selected ? "hidden sm:flex w-72" : "flex w-full sm:w-72"}`}>
            {/* 검색 */}
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="relative shrink-0">
                        <img src={conv.tutorAvatar} alt={conv.tutorName} className="w-11 h-11 rounded-full object-cover" />
                        {conv.online && (
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between mb-0.5">
                          ...
                          <span className="text-[10px] text-muted-foreground shrink-0 ml-2">{last.time.split(" ").pop()}</span>
                        </div>
                        <p className="text-xs text-muted-foreground truncate">{conv.tutorSubject}</p>
                        <p className={`text-xs mt-0.5 truncate ${conv.unread > 0 ? "font-semibold text-foreground" : "text-muted-foreground"}`}>
          ...
          {selected ? (
            <div className="flex flex-col flex-1 min-w-0">
              ...
              <div className="flex items-center gap-3 px-4 py-3.5 border-b border-border shrink-0 bg-card">
                {/* 모바일 뒤로가기 */}
                <button onClick={() => setSelectedId(null)} className="sm:hidden p-1.5 rounded-lg hover:bg-muted transition-colors cursor-pointer text-muted-foreground shrink-0">
                  <ArrowLeft size={16} />
                </button>
                <div className="relative shrink-0">
                  <img src={selected.tutorAvatar} alt={selected.tutorName} className="w-9 h-9 rounded-full object-cover" />
                  {selected.online && (
                    <span className="absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 bg-emerald-400 border-2 border-card rounded-full" />
                  )}
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-bold text-foreground">{selected.tutorName}</p>
                ...
                {selected.messages.map((msg) =>
                  msg.from === "tutor" ? (
                    <div key={msg.id} className="flex items-end gap-2">
                      <img src={selected.tutorAvatar} alt={selected.tutorName}
                ...
                <div className="flex items-center gap-2 bg-muted/60 rounded-xl px-3 py-2">
                  ...
                  <button
                    ...
                    className={`p-1.5 rounded-lg transition-colors shrink-0 ${
                      input.trim() ? "text-primary hover:bg-primary/10 cursor-pointer" : "text-muted-foreground/40 cursor-not-allowed"
                    }`}
                  >
                    <Send size={16} />
                  </button>
            ...
            <div className="hidden sm:flex flex-1 items-center justify-center flex-col gap-3 text-center bg-muted/10">
              ...
              <p className="text-xs text-muted-foreground">왼쪽 목록에서 튜터를 선택하면<br/>채팅을 시작할 수 있습니다.</p>
            </div>
          )}
        </div>
      </div>
