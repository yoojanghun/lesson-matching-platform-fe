...
export default function TutorsPage({ onSelectTutor }: Props) {
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3 flex-wrap">
        <div className="flex items-center bg-card border border-border rounded-lg px-3 gap-2 flex-1 min-w-48">
          ...
          <input
            className="flex-1 bg-transparent text-sm py-2 outline-none text-foreground placeholder-muted-foreground"
            placeholder="튜터 이름, 과목 검색..."
          />
        </div>
        <div className="flex gap-2 flex-wrap">
          {["전체", "피아노", "기타", "바이올린", "보컬", "드럼", "첼로", "작곡"].map((cat) => (
            <button
              key={cat}
